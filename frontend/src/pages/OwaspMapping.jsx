  ...
  return (
    <div>
      ...
      <Panel title="OWASP Severity Heatmap" className="mb-5">
            ...
            <thead>
              <tr>
                ...
                {owaspCategories.map((o) => (
                  <th key={o.id} className="mono text-[11px] text-content-muted font-semibold pb-1">{o.id}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row}>
                  <td className="text-[11px] text-content-dim font-semibold pr-2 uppercase text-right w-20">{row}</td>
                  {owaspCategories.map((o) => {
                    ...
                    return (
                      <td key={o.id}>
                        <button
                          ...
                          style={{
                            backgroundColor: count > 0 ? severityColor[sevKey].bg : 'var(--card2)',
                            border: `1px solid ${count > 0 ? severityColor[sevKey].border : t.grid}`,
                            outline: active ? `2px solid ${t.brand}` : 'none'
                          }}
                        >
                          <span className="text-sm font-bold" style={{ color: count > 0 ? severityColor[sevKey].text : 'var(--text-dim)' }}>
                            {count || '—'}
                          </span>
                        </button>
                      </td>
        ...
        {activeCell && (
          <div className="mt-4 pt-3 border-t border-base-border">
            <div className="text-xs text-content-dim mb-2">Filtered: <span className="mono text-brand">{activeCell.cat}</span> · {activeCell.row}</div>
            <div className="space-y-1.5">
              {findings.filter((f) => f.owasp === activeCell.cat && f.severity === activeCell.row.toUpperCase()).map((f) => (
                <Link key={f.id} to={`/findings/${f.id}`} className="flex items-center gap-3 text-[13px] text-content hover:text-content">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <Panel><div className="label-eyebrow mb-1">Most Affected Category</div><div className="text-lg font-bold text-content">{mostAffected?.id}</div><div className="text-xs text-content-dim">{mostAffected?.name} · {mostAffected?.findings} findings</div></Panel>
        <Panel><div className="label-eyebrow mb-1">Highest Risk Category</div><div className="text-lg font-bold text-sev-critical">{highestRisk?.id}</div><div className="text-xs text-content-dim">{highestRisk?.name} · risk {highestRisk?.risk}</div></Panel>
        <Panel>
          <div className="label-eyebrow mb-1">Newly Detected</div>
          {newlyDetected === null ? (
            <>
              <div className="text-lg font-bold text-content-muted">—</div>
              <div className="text-xs text-content-dim">No previous scan of this target to compare against</div>
            </>
          ) : newlyDetected.length === 0 ? (
            <>
              <div className="text-lg font-bold text-content-muted">None</div>
              <div className="text-xs text-content-dim">No category appeared that the previous run did not already show</div>
            </>
          ) : (
            <>
              <div className="text-lg font-bold text-sev-high">{newlyDetected.map((o) => o.id).join(', ')}</div>
              <div className="text-xs text-content-dim">{newlyDetected.map((o) => o.name).join(' · ')} — first observed this run</div>
            </>
          )}
        </Panel>
      </div>
          ...
          {filteredCats.map((o) => (
            <div key={o.id} className="p-4 rounded-lg border border-base-border bg-base-card2">
              <div className="flex items-center justify-between mb-1">
                <span className="mono font-bold text-content">{o.id}</span>
                <SeverityBadge level={o.severity} />
              </div>
              <div className="text-[13px] text-content mb-3">{o.name}</div>
              <div className="grid grid-cols-2 gap-2 text-[12px] text-content-muted mb-3">
                <div>Findings: <span className="font-bold text-content">{o.findings}</span></div>
                <div>Risk: <span className="font-bold text-content">{o.risk}</span></div>
                <div>Evidence: <span className="font-bold text-content">{o.evidence}</span></div>
                <div>Chains: <span className="font-bold text-content">{o.chains}</span></div>
              </div>
