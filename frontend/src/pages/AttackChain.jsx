...
const statusFor = (t) => ({
  failed: { color: t.critical, bg: 'var(--sev-critical-bg)', border: 'var(--sev-critical-border)', Icon: ShieldOff, word: 'failed' },
  ok: { color: t.low, bg: 'var(--sev-low-bg)', border: 'var(--sev-low-border)', Icon: ShieldCheck, word: 'held' },
  info: { color: t.neutral, bg: 'var(--sev-neutral-bg)', border: 'var(--border2)', Icon: Circle, word: 'observed' }
})
    ...
    <div className="flex gap-4">
      ...
      <div className="pb-7 min-w-0 flex-1">
        <div className="flex items-baseline gap-2 flex-wrap mb-1">
          <span className="mono text-[11px] text-content-dim">PHASE {phase.n}</span>
          <span className="text-[15px] font-bold text-content">{phase.name}</span>
          <span
        ...
        <p className="text-[13px] text-content-muted max-w-[70ch]">{phase.detail}</p>

        {phase.data?.text && (
          <div className="mt-2.5">
            <div className="text-[10px] uppercase tracking-wide text-content-dim mb-1">
              {phase.data.label}
            </div>
            <pre className="mono text-[12px] text-content bg-base-bg rounded-lg p-3 border border-base-border whitespace-pre-wrap break-words max-h-40 overflow-y-auto">
{phase.data.text}
            </pre>
          </div>
        )}
  ...
  if (!chain) {
    return (
      <div>
        ...
        <Panel><div className="text-sm text-content-dim py-6 text-center">No attack chains in this scan.</div></Panel>
      </div>
    )
  }
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-left transition-colors border ${
                i === selected
                  ...
                  : 'bg-base-card text-content-muted hover:text-content border-base-border'
              }`}
            >
              <div className="text-[12px] font-bold truncate max-w-[220px]">{c.findingTitle}</div>
              <div className={`text-[10px] mono ${i === selected ? 'text-white/70' : 'text-content-dim'}`}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
        <Panel title="Attack path" className="xl:col-span-2">
          ...
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-base-border flex-wrap text-[11px]">
            ...
            <span className="flex items-center gap-1.5 text-content-muted">
              <span className="w-3 h-0.5 inline-block" style={{ background: t.rail }} /> stopped here
            </span>
            <span className="text-content-dim ml-auto">click any phase to jump to its detail</span>
          </div>
        </Panel>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
        <Panel title="Attack sequence" className="xl:col-span-2">
          <div className="mb-5 p-3 rounded-lg border border-base-border bg-base-card2">
            <div className="text-[13px] font-bold text-content mb-0.5">{chain.findingTitle}</div>
        ...
        <Panel title="Where it broke down">
          ...
          <div className="space-y-2">
            {chain.phases.map((p) => {
              ...
              return (
                ...
                >
                  <span className="mono text-[11px] text-content-dim w-4 shrink-0">{p.n}</span>
                  <span className="text-[13px] text-content flex-1 min-w-0 truncate">{p.name}</span>
          ...
          <div className="mt-5 pt-4 border-t border-base-border">
            <div className="text-[11px] uppercase tracking-wide text-content-dim mb-1.5">Remediation</div>
      ...
      <Panel title="Runtime activity">
        ...
        <div className="space-y-1.5">
          {attackTimeline.length === 0 && (
            <div className="text-sm text-content-dim py-4 text-center">
              The target exposed no runtime telemetry for this scan.
            </div>
          )}
          {attackTimeline.slice(0, 25).map((row, i) => (
            <div
              key={i}
              className="flex items-start gap-3 p-2.5 rounded-lg border border-base-border hover:bg-base-card2 transition-colors"
            >
              <span className="mono text-[11px] text-content-dim shrink-0 w-[70px]">{row.time}</span>
              {row.kind === 'tool_call'
                ? <Zap size={14} className="shrink-0 mt-0.5" style={{ color: row.critical ? '#ef4444' : t.medium }} />
                : <Circle size={8} className="shrink-0 mt-1.5 text-content-dim" />}
              <span className="min-w-0 flex-1">
                <span className="block text-[13px] text-content">
                  {row.label}
                  {row.note && <span className="mono text-[11px] text-content-dim ml-2">{row.note}</span>}
                </span>
                {row.detail && (
                  <span className="block mono text-[11px] text-content-dim truncate">{row.detail}</span>
                )}
                {row.prompt && (
                  <span className="block mono text-[11px] text-content-dim truncate mt-0.5">
                    <ChevronRight size={10} className="inline" /> caused by: "{row.prompt}"
                  </span>
                )}
              </span>
            </div>
